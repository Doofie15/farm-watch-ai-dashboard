...
const ConfigStatusesPage: React.FC = () => {
  ...
  const handleUpdate = (id: string, newLabel: string, newIsActive: boolean) => {
    const updatedItems = config.statuses.map(item =>
      item.id === id ? { ...item, label: newLabel, isActive: newIsActive } : item
    );
    updateConfigList('statuses', updatedItems);
  };

  const handleAddItem = (id: string, label: string) => {
    addConfigListItem('statuses', id, label);
  };

  const handleDeleteItem = (itemId: string) => {
    deleteConfigListItem('statuses', itemId);
  };
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <SettingsCategory title="Statuses Configuration">
        <p className="text-xs text-slate-500 mb-3">Define the possible statuses for an incident. Edit labels, toggle active status, or add new statuses. System-defined statuses cannot be deleted.</p>
        <ConfigurableListEditor
          items={config.statuses}
          onUpdateItem={handleUpdate}
          onAddItem={handleAddItem}
          onDeleteItem={handleDeleteItem}
          listKeyFriendlyName="Status"
        />
      </SettingsCategory>
...
export default ConfigStatusesPage;